...
// RPC proxy endpoint handler to bypass browser CORS rules on Vercel
...
export function useFlightDelayVault() {
  ...
  const fetchClaimsState = useCallback(async () => {
    ...
    try {
      ...
      console.error('Fetch claims state error:', err);
      setError(err.message || 'Failed to fetch claims from GenLayer contract');
    } finally {
      setLoading(false);
    }
  }, []);
